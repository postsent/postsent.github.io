...
The sky above the cloud - MIT future plan on cloud computing, great name.
...
\#cs github recommend personal token access over auth with password when use a thrid device to access repo, realise it is pretty obvious why.

# 26

No udpates recently due to yet another really long assigNment - YARN - cloud computing

Revision time, w13.

\#cs always make meaningful variable name and comment as if I hit by a car, lost all memory but still can recall based on them.


\#cs torch santiy check training dataset should contain more elems elsewise, unexpected classifiaction_report errors since only 10 is trained...

\#lesson I change the stack layer to 3, before is 1, and then forget to change it back, which makes the pad to sequence changes applied give 0 accuracy and I thought it is because 
the newly added changes and ofc it turns out to be an issue of the stack layer.

# 27

let think step by step - "magic word is all you need"
